 ...
 */
{
	...
	int buflen =
	    MultiByteToWideChar(CP_ACP, 0, (PCSTR) device, -1, (LPWSTR) ComName,
				0);
	MultiByteToWideChar(CP_ACP, 0, (PCSTR) device, -1, (LPWSTR) ComName,
			    buflen);
...
*/
{
	if (src->recv.bufpos >= src->recv.bufused) {
		for (;;) {
			int n =
			    serial_read(src, non_blocking, src->recv.buffer,
...
*/
{
	...
	struct pargs args;
	struct {
		void (*add) (struct pargs * args, const char *name,
			     int baudrate);
		struct pargs *x;
	} Platform = {
	padd, &args};
