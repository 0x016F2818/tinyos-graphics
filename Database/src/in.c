...
int update_node_info(MYSQL *mysql,node_t info){
    ...
    sprintf(command,"call update_node_info_sp(%d,%d,'work','normal',%d,point(%f,%f))",info.node_id,info.parent_id,info.power,info.position.x,info.position.y);
...
int insert_sense_record(MYSQL *mysql,sensor_t info){
    ...
    sprintf(command,"call insert_sense_record_sp(%d,%lf,%lf,%lf,%lf,%lf,%lf,%lf,NULL,NULL,NULL,NULL)",info.node_id,info.temp,info.photo,info.sound,info.x_acc,info.y_acc,info.x_mag,info.y_mag);
