...
create procedure update_node_info_sp(in id int,in parent_id int,
...
begin
    ...
    if amount = 0 then
        insert into node_tb(node_id,parent_id,work_state,node_status,power,GPS) values(id,parent_id,state_num,status_num,power,GPS);
    else
        update node_tb
        set work_state = state_num,
            node_status = status_num,
            node_tb.parent_id = parent_id,
            node_tb.power = power,
            node_tb.GPS = GPS
        where node_id = id;
    end if;
