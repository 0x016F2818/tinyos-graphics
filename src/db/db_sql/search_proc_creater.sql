...
begin
    ...
    from node_viw,work_state_viw,node_status_viw,network_viw
            ...
            and node_viw.node_id        = network_viw.node_id
...
if id < 0 and sense is null then
    ...
    pressure,accelerate_x,accelerate_y,shoke,sense_time from sense_tb
...
top:begin 
    if id < 0 and sense is null then
        select temperature,humidity,brightness,microphone,terre_mag_x,terre_mag_y,
                pressure,accelerate_x,accelerate_y,shoke from sense_viw_
        where sense_viw.insert_time >= start_time and sense_viw.insert_time <= end_time;

    elseif id > 0 and sense is null then
        select temperature,humidity,brightness,microphone,terre_mag_x,terre_mag_y,
                pressure,accelerate_x,accelerate_y,shoke from sense_tb
...
/*##########################################################################*/
/*drop procedure if exists sp_get_relative_record(in id int,in sense varchar(30),in start_time timestamp,in numbers bigint );*/
/*create procedure sp_get_relative_record(in id int,in sense varchar(30),in start_time timestamp,in numbers bigint)*/
/*begin*/
/*set @id         = id;*/
/*set @sense      = sense;*/
/*set @start_time = start_time;*/
/*set @numberst   = numbers;*/


/*prepare a1 from 'select temperature,brightness,microphone from sense_viw where insert_time >= ? limit ?';*/
/*prepare a2 from 'select temperature,brightness,microphone from sense_viw where insert_time <= ? limit ?';*/
/*prepare b2 from 'select temperature,brightness,microphone from sense_viw where node_id = ? and insert_time >= ? limit ?';*/
/*prepare b2 from 'select temperature,brightness,microphone from sense_viw where node_id = ? and insert_time <= ? limit ?';*/

/*prepare a1 from 'select temperature from temperature_viw where insert_time >= ? limit ?';*/
/*prepare a2 from 'select temperature from temperature_viw where insert_time <= ? limit ?';*/
/*prepare b2 from 'select temperature from temperature_viw where node_id = ? and insert_time >= ? limit ?';*/
/*prepare b2 from 'select temperature from temperature_viw where node_id = ? and insert_time <= ? limit ?';*/

/*prepare a1 from 'select brightness from brightness_viw where insert_time >= ? limit ?';*/
/*prepare a2 from 'select brightness from brightness_viw where insert_time <= ? limit ?';*/
/*prepare b2 from 'select brightness from brightness_viw where node_id = ? and insert_time >= ? limit ?';*/
/*prepare b2 from 'select brightness from brightness_viw where node_id = ? and insert_time <= ? limit ?';*/

/*prepare a1 from 'select microphone from microphone_viw where insert_time >= ? limit ?';*/
/*prepare a2 from 'select microphone from microphone_viw where insert_time <= ? limit ?';*/
/*prepare b2 from 'select microphone from microphone_viw where node_id = ? and insert_time >= ? limit ?';*/
/*prepare b2 from 'select microphone from microphone_viw where node_id = ? and insert_time <= ? limit ?';*/
