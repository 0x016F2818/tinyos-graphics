...
void open_serial(const char *dev, int baud)
{
	...
	if (strncasecmp(dev, "COM", 3) == 0) {
		fprintf(stderr,
			"Warning: you're attempting to open a Windows rather that a Cygwin device.  Retrying with ");
		portnum = atoi(dev + 3);
...
int do_child(void)
{
	...
	/* 打开串口设备 */
	open_serial("/dev/ttyUSB1", platform_baud_rate("57600"));
	/* 得到串口设备的fd */
	serfd = serial_source_fd(src);
	/* 打开本地套接字， 端口号argv[1] */
	open_server_socket(8989);
	...
	for (;;) {
		...
		/* 将文件描述符加入到描述符集中， 包括:串口fd， 服务器套接字fd， 若干客户端的fd */
		FD_ZERO(&rfds);
		...
		/* 如果有数据，那么等待, 否则设置select最后一个参数为zero， 完全不等待 */
		if (serial_empty) {
